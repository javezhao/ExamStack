		...
		for (var i = 0; i < questions.length; i++) {
			...
			if ($(questions[i]).hasClass("qt-singlechoice")) {
				...
				if (radio_checked.length == 0) {
					...
					//  获取选项对应的具体内容
					//var detail = $('questions[i]:radio:checked').val();
				//	 var detail = $(questions[i]).find("span[name='data-ref']").eq(current_index).val();
				//	var detail = $(questions[i]).value();
				//	 console.log(detail,$(questions[i]).find("data-ref").eq(current_index),current_index);
					
				//	var detail = $(questions[i]).find("#answer").val();
				//	var detail = $(".question-input"):checked.val();
				
					
					answerSheetItem.answerDetail="单选页面返回的值singlechoice  ";
					
					console.log(answerSheetItem.answerDetail);
				}
				...
				if (checkbox_checked.length == 0) {
					...
					for (var l = 0; l < checkbox_checked.length; l++) {
						...
						// zwj 把每个选项的具体内容添加
					//	checkbox_checked[tm_answer].value;
					//	answerDetail += tm_answer;
					}
					answerSheetItem.answer = tm_answer;
					answerSheetItem.answerDetail="  多选页面返回的值 multiplechoice "+answerDetail;
