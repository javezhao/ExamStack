...
public class ExamPage {
	...
	public String examStartPage(Model model, HttpServletRequest request, @PathVariable("examId") int examId) {
		...
		//zwj add
		System.out.println("ExamPage zwjadd strUrl---"+strUrl);
		System.out.println("ExamPage zwjadd content---"+content);
		...
		ArrayList<ChoiceQuestion> singleChoiceQuestionList = new ArrayList<ChoiceQuestion>(); //zwj add	
		ArrayList<ChoiceQuestion> multipleChoiceQuestionList = new ArrayList<ChoiceQuestion>(); //zwj add	
//		System.out.println("ExamPage questionList length= " +questionList.size());
		
		for (QuestionQueryResult question : questionList) {
			System.out.println("ExamPage zwjadd question---"+question); //zwj add
			
			QuestionAdapter adapter = new QuestionAdapter(question, strUrl);
		//	sb.append(adapter.getUserExamPaper());  // 以前的代码
			
			if(question.getQuestionTypeId() == 1)  // 单选题要乱序
			{				
				singleChoiceQuestionList.add(adapter.getQuestion()); //将每个题目添加到列表中
			//	 将正确答案的选项，对应的具体内容，保存到数据库，后面乱序后，要比较
				int questionId = adapter.getQuestion().getQuestionId();
				String answerDetail = adapter.getQuestion().getAnswerDetail();
				System.out.println("ExamPage singleChoiceQuestionList questionId " +questionId
						+" answerDetail---"+ answerDetail); 
				 examPaperService.updateAnswerDetail(questionId, answerDetail);
			}
			else if(question.getQuestionTypeId() == 2)  // 多选题要乱序
			{
				multipleChoiceQuestionList.add(adapter.getQuestion()); //将每个题目添加到列表中
				// 将正确答案的选项，对应的具体内容，保存到数据库，后面乱序后，要比较
				int questionId = adapter.getQuestion().getQuestionId();
				String answerDetail = adapter.getQuestion().getAnswerDetail();
				System.out.println("ExamPage multipleChoiceQuestionList questionId " +questionId
						+" answerDetail---"+ answerDetail); 
				 examPaperService.updateAnswerDetail(questionId, answerDetail);
			}
			else {
				sb.append(adapter.getUserExamPaper());  // 不是单选题，按以前的写法
			}
			
		}
		
		// 遍历
//		for (int idx = 0; idx < singleChoiceQuestionList.size(); idx++) {
//			Map<String, String> option = singleChoiceQuestionList.get(idx).getOption();
//			for (String key : option.keySet()) {
//				String value = option.get(key);
//				System.out.println(" ExamPage singleChoiceQuestionList ---questionId="
//						+singleChoiceQuestionList.get(idx).getQuestionId()
//				    + "  get(key()="+ key+ "  value= "  +value); //zwj add 获取选项的内容
//			}
//		}
//	
		
		if(singleChoiceQuestionList.size() > 0)   
		{
			QuestionAdapter adapter = new QuestionAdapter();
			sb.append(adapter.DisorderQuestions(singleChoiceQuestionList, 1));  // 乱序题目		
		}  
		if(multipleChoiceQuestionList.size() > 0)   
		{
			QuestionAdapter adapter = new QuestionAdapter();
			sb.append(adapter.DisorderQuestions(multipleChoiceQuestionList, 2));  // 乱序题目		
		}  
	...
	public String examFinishedPage(Model model,@PathVariable("examId") int examId) {
		...
		System.out.println("ExamPage examFinishedPage zwj examId="+ examId); // zwj add
		System.out.println("ExamPage examFinishedPage zwj userInfo.getUserid()="+ userInfo.getUserid()); // zwj add
