...
public class ExamAction {
	...
	public @ResponseBody Message finishExam(@RequestBody AnswerSheet answerSheet) {
		...
		System.out.println("ExamAction finishExam zwj called="); // zwj add
		...
		try {
			...
			System.out.println("ExamAction finishExam zwj getExamPaperById="+answerSheet.getExamPaperId()); // zwj add
			...
			for(AnswerSheetItem item : target.getAnswerSheetItems()){
				...
				System.out.println(" 原始题目item.getQuestionId()--= "
				+item.getQuestionId() 
				+"  item.getAnswer()= "+item.getAnswer()
				+ "  item.getAnswerDetail()= " +item.getAnswerDetail());
			}
			...
			for(AnswerSheetItem item : answerSheet.getAnswerSheetItems()){
				// 正确的答案内容,从原数据库取得
				String rightAnswerDetail = examPaperService.getAnswerDetail(item.getQuestionId());

				System.out.println(" 写的答案= " +item.getAnswer() 
						+"  写的答案内容："+ item.getAnswerDetail()
						+ " 正确答案id：" +item.getQuestionId() 
						+ " 正确选项: " +answerMap.get(item.getQuestionId()).getAnswer()
				     + " 正确答案内容: " + rightAnswerDetail);
			// 获取该QuestionId对应的答案具体内容
				
				
			//	if(item.getAnswer().equals(answerMap.get(item.getQuestionId()).getAnswer())){
				// 选项乱序后，比较答案具体内容
				if(item.getAnswerDetail()!=null && (item.getAnswerDetail().trim()).equals(rightAnswerDetail.trim())){		
					...
					item.setRight(true);
					System.out.println(" 做对的题目是= " +item.getQuestionId());
				}
			}
			//6、将处理结果写入考试历史中
