$(function () {
  var up = $('#uploadify').Huploadify({
    'debug': false,
    'buttonText': '点击上传附件',
    'uploader': document.getElementsByTagName('base')[0].href + 'secure/uploadUser-uploadify/',
    'queueID': 'fileQueue',
    'multi': false,
    'auto': true,
    'fileType' : '.xlsx',
  ...
  $('#from-student-import').submit(function () {
...
//			return false
  ...
  //     alert($("#from-student-import").attr("action"));
   // document.write($("#from-student-import").attr("action"));
  ...
  //    document.write($filePath);
    $.ajax({
      ...
      type: 'POST',
      url: $('#from-student-import').attr('action'),
   //   url: $('#from-question-import').attr('action') + '/3' ,
       ...
       //  alert('AJAX 请求完成')
        ...
        if (message.result == 'success') {
          util.success('导入成功', function () {
            ...
            document.location.href = document.getElementsByTagName('base')[0].href + '/admin/user/student-list'
          })
        } else {
