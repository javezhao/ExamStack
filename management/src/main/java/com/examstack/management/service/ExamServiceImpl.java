	...
	public void addExam(Exam exam) {
		...
		try {
			...
			if(exam.getGroupIdList() != null && exam.getGroupIdList().size() > 0){
				List<User> userList = userMapper.getUserListByGroupIdList(exam.getGroupIdList(), null); // group 实际指 department 单位名分组的
			//	List<User> userList = userMapper.getUserListByDepartmentIdList(exam.getGroupIdList(), null);
