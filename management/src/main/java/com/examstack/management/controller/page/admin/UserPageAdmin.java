...
public class UserPageAdmin {
	...
	*/
		...
		List<User> userList = userService.getUserListByDepartmentIdAndParams(departmentId, "ROLE_STUDENT", searchStr, page);
		String pageStr = PagingUtil.getPagelink(index, page.getTotalPage(), "", "admin/user-list");
		List<Department> departmentList = userService.getDepartListByUserId(userInfo.getUserid(), null);
		...
		model.addAttribute("departmentList", departmentList); //add
	//	model.addAttribute("groupList", groupList);

		System.out.println("UserPageAdmin  userListPage ....");
		return "user-list";
	}
	...
	@RequestMapping(value = { "admin/user/inner/user-list/{departmentId}-{authority}" }, method = RequestMethod.GET)
	public String showUserListInnerAdminPage(Model model, HttpServletRequest request, @PathVariable Integer departmentId, @PathVariable String authority) {
		UserInfo userInfo = (UserInfo) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
		int index = 1;
		if (request.getParameter("page") != null)
			index = Integer.parseInt(request.getParameter("page"));
		...
		if(request.getParameter("searchStr") != null){
			searchStr = request.getParameter("searchStr");
			System.out.println(" showUserListInnerAdminPage 111 searchStr =" + searchStr);

		}
		List<User> userList = userService.getUserListByDepartmentIdAndParams(departmentId, authority, searchStr, page);
		String pageStr = PagingUtil.getPagelink(index, page.getTotalPage(), "&searchStr=" + searchStr, "admin/user/inner/user-list/" + departmentId + "-" + authority);
	...
	//	model.addAttribute("groupList", groupList);
		...
		System.out.println(" showUserListInnerAdminPage  searchStr =" + searchStr);
	...
	*/
	 ...
	 * @return
	 */
	@RequestMapping(value = "/admin/user/student-import", method = RequestMethod.GET)
	public String studentImportPage(Model model) {
		System.out.println("UserPageAdmin studentImportPage  zwj ....");
